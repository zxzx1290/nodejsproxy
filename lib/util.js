...
function getPrefixURL(domain, type) {
    if (typeof config.frontendTunnel[domain] === 'undefined') {
        console.log(domain+':not found');
        return '/';
    } else if (typeof config.frontendTunnel[domain][type] === 'undefined') {
        return '/';
    } else {
...
function checkUser(host, username, password) {
...
function loginSuccess(client, proxysession, username, lognlive, req, res, callback) {
...
function sendLineBot(text) {
    if(config.loginNotify !== null) {
        request({
            method: 'GET',
            url: config.loginNotify + encodeURIComponent(text)
